...
function handleDefaultVolume(video, volume, volumeIncrement) {
    ...
    video.dataset.volume = (volume / 100).toString();

    video.addEventListener("volumechange", () => {
        // Checks to see if the registered change in volume is equal to the increment. If it is not then it is denied.
        if (!(video.volume === video.dataset.volume - volumeIncrement || video.volume === video.dataset.volume + volumeIncrement || video.volume.toString() === video.dataset.volume)) {
...
getUserSettings().then(settings => {
    ...
    let onScroll = function (event) {
        ...
        for (const element of elements) {
            ...
            video.volume = volume;
            video.dataset.volume = volume.toString();

            //Update overlay text
            let div = document.getElementById("volumeOverlay");
            div.innerHTML = Math.round(video.volume * 100).toString();
